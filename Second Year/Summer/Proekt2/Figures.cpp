...
void Figures::open(dynamicArray<char>& filename){
...
//private
void Figures::formatNewLineFigure(dynamicArray<char>& newLine){
    // formating = приемаме че са подредени и стойностите са в кавички 
                
    dynamicArray<char> formattedLine;
    ...
    while(newLine[i] != ' ') {
        formattedLine.pushBack(newLine[i]);
        i++;
    }

    formattedLine.pushBack(newLine[i]); //for ' '
    i++;
...
void Figures::addFigure(dynamicArray<char>& niz){
